...
# Exlore abundances in the data set
...
glm8 <- glm(corales_algunos_sum ~  Ascidiacea * algae_richnes , family = poisson(), data = corales)

glm9 <- glm(corales_algunos_sum ~  Holothuroidea *  algae_richnes, family = poisson(), data = corales)

glm10 <- glm(corales_algunos_sum ~  Bivalvia *  algae_richnes, family = poisson(), data = corales)

glm11 <- glm(corales_algunos_sum ~  Mollusca *  algae_richnes, family = poisson(), data = corales)

glm12 <- glm(corales_algunos_sum ~  Polychaeta * algae_richnes, family = poisson(), data = corales)

glm13 <- glm(corales_algunos_sum ~  algae_richnes, family = poisson(), data = corales)

glm14 <- glm(corales_algunos_sum ~  Polychaeta, family = poisson(), data = corales)

glm15 <- glm(corales_algunos_sum ~  Porifera, family = poisson(), data = corales)

glm16 <- glm(corales_algunos_sum ~  Echinoidea, family = poisson(), data = corales)




# anova(glm7, glm8, glm9, glm10, glm11, glm12)
# AIC(glm7, glm8, glm9, glm10, glm11, glm12)



AICctab(glm7, glm8, glm9, glm10, glm11, 
        glm12, glm13, glm14, glm15, glm16,
        base=T, weights=T, delta=T, sort=T,
    nobs = length(corales))

# 
# #### automatic procedure con 1st order
# global.model <-  glm(corales_algunos_sum ~ # number of CORALS
#                        algae_richnes + # 
#                        Sediment + # c
#                        Mollusca + # 
#                        Ascidiacea + # fish species richness
#                        Porifera, # total number of visitors
#                        data=corales) # data frame we got this from
# 
...
newdato <- as.data.frame(cbind(Sediment=rep(0:1, each = 10), algae_richnes=rep(1:10, 10)) )
predichos <- predict(glm9, newdata = newdato, type = "response")

# 
# # plot predictions but not as surface
# sjp.glm(glm12, type = "pred", vars = c("Polychaeta", "algae_richnes"), show.ci = TRUE)
# 
# sjp.glm(glm12, type = "pred", vars = c("Sediment", "algae_richnes"), show.ci = TRUE, facet.grid = FALSE)
# 
...
grid = expand.grid(list(Polychaeta = seq(0, 1, 0.1), algae_richnes = seq(0, 6, 1)))

z = predict(glm12, newdata = grid)
...
pr.mat<-data.frame(Polychaeta=seq(0, 
                                1, 
                                length.out=100),
                   algae_richnes=seq(0, 6, 
                            length.out=100))

for(i in 1:100){
  for(j in 1:100){
       psi.matrix[i, j]<-predict(glm12, newdata=data.frame(
...
par(mar=c(3,5,3,2))
image.plot(x = pr.mat$Polychaeta, y = pr.mat$algae_richnes , z = psi.matrix,
           col = mapPalette(100), axes=F,xlab = NA ,ylab = NA, 
           legend.width=1, axis.args=list(cex.axis=0.8, lwd=0,
                              line=-0.5), main="Coral recruitment")
contour(x = pr.mat$Polychaeta, y = pr.mat$algae_richnes, z = psi.matrix, 
        add = TRUE, lwd = 1)
...
points(corales$Polychaeta, corales$algae_richnes, pch=19, cex=0.5, 
       col="black")
...
m11.cv <- cv.glm(data = corales, glm12, cost, K = 10)  # use leave-one-out cross validation (can use K-fold cross validation for larger data sets)
...
muhat <- fitted(glm12)
glm12.diag <- glm.diag(glm12)
(cv.err <- mean((glm12$y - muhat)^2/(1 - glm12.diag$h)^2))

glm.diag.plots(glm12, glm12.diag)
...
m.roc <- multiclass.roc(corales$corales_algunos_sum, predict(glm12, backtransform = TRUE))
auc(m.roc)
ci(m.roc)
...
############################################################
# Este codigo no se evalua... no lo borre para dejarlo como 
# referencia. 
############################################################
